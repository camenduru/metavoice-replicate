...
# !python fam/llm/sample.py --huggingface_repo_id="metavoiceio/metavoice-1B-v0.1" --spk_cond_path="assets/ava.flac" --dtype=float16
...
class SamplingControllerConfig:
    ...
    spk_cond_path: str = "/content/metavoice-src/ava.flac"
...
class Predictor(BasePredictor):
    def setup(self) -> None:
        ...
        config_first_stage = InferenceConfig(
            ckpt_path=self.first_stage_ckpt_path,
            ...
            compile=self.sampling_config.compile,
            init_from=self.sampling_config.init_from,
            output_dir=self.sampling_config.output_dir,
        )

        config_second_stage = InferenceConfig(
            ckpt_path=self.second_stage_ckpt_path,
            num_samples=self.sampling_config.num_samples,
            seed=self.sampling_config.seed,
            device=self.sampling_config.device,
            dtype=self.sampling_config.dtype,
    ...
    def predict(
        ...
        output_audio = sample_utterance(
            self.sampling_config.text,
            os.path.expanduser(self.sampling_config.spk_cond_path),
            self.smodel,
            self.llm_first_stage,
            self.llm_second_stage,
